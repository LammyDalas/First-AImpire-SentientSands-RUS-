...
#include "../RE_Kenshi_Source/KenshiLib/Include/kenshi/Bounty.h"
#include "../RE_Kenshi_Source/KenshiLib/Include/kenshi/BountyManager.h"
#include "../RE_Kenshi_Source/KenshiLib/Include/kenshi/CharStats.h"
#include "../RE_Kenshi_Source/KenshiLib/Include/kenshi/Enums.h"
#include "../RE_Kenshi_Source/KenshiLib/Include/kenshi/StateBroadcastData.h"
...
// Lowercase a UTF-8 string, Cyrillic included.
//
// std::tolower only knows ASCII, so Russian names came out of it unchanged and
// every keyword comparison below silently failed. Cyrillic in UTF-8 is two
// bytes: 0xD0 0x90..0xBF for А..Я and 0xD0 0xB0..0xBF / 0xD1 0x80..0x8F for
// а..я. Ё (0xD0 0x81) maps to ё (0xD1 0x91).
static std::string ToLowerUtf8(const std::string &in) {
  std::string out;
  out.reserve(in.size());
...
// Strip Kenshi gender markup from a display name.
//
// The game stores gendered roles as "Трактирщи/кца1/" and expands the slash
// group at draw time. Whichever form reaches us, both are reduced to the same
// stem so a single list entry matches male and female alike.
static std::string StripGenderMarkup(const std::string &in) {
  std::string out;
  out.reserve(in.size());
  bool inGroup = false;
  for (size_t i = 0; i < in.size(); ++i) {
  ...
  // Trim trailing whitespace left behind by a removed group.
...
static bool IsGenericName(Character *npc, const std::string &name) {
  ...
  // Gendered roles are stored as "Трактирщи/кца1/" and expanded at draw time,
  // so compare the stripped stems as well — otherwise barmen, fences and every
  // other gendered role never match their own template.
  if (npc->getGameData() && !npc->getGameData()->name.empty()) {
    const std::string &tpl = npc->getGameData()->name;
    if (name == tpl)
      return true;
    std::string nameStem = StripGenderMarkup(name);
    std::string tplStem = StripGenderMarkup(tpl);
    if (!nameStem.empty() && !tplStem.empty()) {
      std::string nameLow = ToLowerUtf8(nameStem);
      std::string tplLow = ToLowerUtf8(tplStem);
      if (nameLow == tplLow)
        return true;
      // "Трактирщик" vs stem "Трактирщи": the display name may carry a gender
      // ending the stem lacks. Allow only a couple of extra bytes and never a
      // space, so an already-renamed "Часовой Дирт" is not caught by "Часовой".
      if (nameLow.size() > tplLow.size() &&
          nameLow.size() - tplLow.size() <= 4 &&
          nameLow.compare(0, tplLow.size(), tplLow) == 0 &&
          nameLow.find(' ', tplLow.size()) == std::string::npos)
        return true;
    }
  }
  ...
  if (!g_genericPrefixes.empty() || !g_genericKeywords.empty()) {
    std::string lowerName = ToLowerUtf8(StripGenderMarkup(name));
  ...
  std::string lowerName = ToLowerUtf8(name);
  for (int i = 0; GENERIC_NAME_PREFIXES[i] != 0; ++i) {
    std::string lowP = ToLowerUtf8(GENERIC_NAME_PREFIXES[i]);
    ...
    while (!g_messageQueue.empty()) {
          ...
          if (command == "TRIGGER_AMBIENT") {
            ...
            if (pipe != std::string::npos) {
              ...
              while ((next = pList.find(",", cur)) != std::string::npos) {
                std::string p = ToLowerUtf8(pList.substr(cur, next - cur));
                g_genericPrefixes.push_back(p);
                cur = next + 1;
              }
              if (cur < pList.length()) {
                std::string p = ToLowerUtf8(pList.substr(cur));
                g_genericPrefixes.push_back(p);
              }
              ...
              while ((next = kList.find(",", cur)) != std::string::npos) {
                std::string k = ToLowerUtf8(kList.substr(cur, next - cur));
                g_genericKeywords.push_back(k);
                cur = next + 1;
              }
              if (cur < kList.length()) {
                std::string k = ToLowerUtf8(kList.substr(cur));
                g_genericKeywords.push_back(k);
              }
      ...
      if (isNPCAction || isNPCSay) {
        ...
        while (true) {
          ...
          if (actStr.find("JOIN_PARTY") != std::string::npos) {
          ...
          } else if (actStr.find("SHOP_DEBUG") != std::string::npos) {
            // Разведочная команда /stock: только осмотр и запись в лог.
            EnterCriticalSection(&g_uiMutex);
            QueuedAction act;
            act.type = ACT_SHOP_DEBUG;
            act.actor = targetHand;
            act.target = g_lastChattingPlayerHand;
            act.taskValue = 0;
            g_uiActionQueue.push_back(act);
            ...
            g_uiActionQueue.push_back(act);
            LeaveCriticalSection(&g_uiMutex);
          } else if (actStr.find("SURRENDER") != std::string::npos) {
            // Сложить оружие. Одной смены задачи мало: боец с горящей
            // враждой всё равно кидается в драку. Поэтому гасим вражду к
            // игроку и его людям, а уже потом ставим «замри».
            EnterCriticalSection(&g_uiMutex);
            QueuedAction act;
            act.type = ACT_SURRENDER;
            act.actor = targetHand;
            ...
            g_uiActionQueue.push_back(act);
            LeaveCriticalSection(&g_uiMutex);
          } else if (actStr.find("RELEASE_SELF") != std::string::npos) {
            // Free the speaker, not the player. RELEASE_PRISONER below is
            // hard-wired to the player character, which is right when an
            // NPC lets the player out of a cage — but useless for a slave
            // the player has just bought, who has to leave his own cage.
            EnterCriticalSection(&g_uiMutex);
            QueuedAction act;
...
void playerUpdate_hook(PlayerInterface *thisptr) {
  ...
  // Keep wiping the runaway bounty off slaves the player has bought.
  // The engine assigns it a moment after the cage opens, so a single pass
  // at release time misses it and the guards come running.
  if (!g_freedSlaves.empty()) {
    for (size_t fi = 0; fi < g_freedSlaves.size();) {
      FreedSlaveWatch &w = g_freedSlaves[fi];
      Character *fc = nullptr;
      try {
        fc = w.who.getCharacter();
      } catch (...) {
        fc = nullptr;
      }
      if (fc && (uintptr_t)fc > 0x1000) {
        try {
          // Keep the freed state pinned: the engine can flip it back to
          // ESCAPING_SLAVE while guards are still assessing the scene.
          StateBroadcastData *sbw = fc->getStateBroadcast();
          if (sbw && sbw->getSlaveState() != NOT_SLAVE) {
            sbw->setSlaveState(NOT_SLAVE);
            sbw->isEscapedPrisoner = false;
            sbw->looksLike_escapedSlave = 0.0f;
            sbw->looksLike_bounty = 0.0f;
            sbw->isSlaveOf = nullptr;
            Log("BOUNTY_WATCH: re-applied NOT_SLAVE to " + fc->getName());
          }
          if (fc->isSlave() == IS_SLAVE || fc->isSlave() == ESCAPING_SLAVE) {
            fc->changeSlaveOwner(hand());
            fc->slaveOwner = hand();
            fc->setSlaveAIJob(false);
          }
          int b = fc->crimes.getTotalBounty();
          if (b > 0) {
            std::vector<Faction *> fl;
            ogre_unordered_map<Faction *, Bounty>::type::iterator bit =
                fc->crimes.bounties.begin();
            for (; bit != fc->crimes.bounties.end(); ++bit) {
              if (bit->first)
                fl.push_back(bit->first);
            }
            for (size_t bi = 0; bi < fl.size(); ++bi)
              fc->crimes.clearBounty(fl[bi]);
            Log("BOUNTY_WATCH: cleared " + ToString(b) + " on " +
                fc->getName());
          }
          fc->crimes.committingCrime = CRIME_NONE;
          fc->crimes.crimeAgainstFaction = nullptr;
          fc->crimes.crimeExpiry = 0.0f;

          // Witnesses keep tagging both of them for a while after the
          // cage opens, so keep untagging for as long as we watch.
          if (ppWorld && *ppWorld) {
            Character *buyer2 = nullptr;
            GameWorld *bw2 = *ppWorld;
            if (bw2->player && bw2->player->playerCharacters.size() > 0)
              buyer2 = bw2->player->playerCharacters[0];
            lektor<RootObject *> wit;
            bw2->getCharactersWithinSphere(wit, fc->getPosition(), 60.0f,
                                          0.0f, 0.0f, 100, 0, nullptr);
            for (uint32_t wi = 0; wi < wit.size(); ++wi) {
              Character *ob = (Character *)wit.stuff[wi];
              if (!ob || (uintptr_t)ob < 0x1000)
                continue;
              try {
                ob->clearTempEnemyStatus(fc);
                if (buyer2)
                  ob->clearTempEnemyStatus(buyer2);
      ...
      // The freeing itself is charged to the player. Note this hook gets a
      // PlayerInterface, which has no player list — the squad lives on
      // GameWorld, reached through the global world pointer.
      try {
        GameWorld *bw = (ppWorld ? *ppWorld : nullptr);
        if (bw && bw->player && bw->player->playerCharacters.size() > 0) {
          Character *pc = bw->player->playerCharacters[0];
          if (pc && pc->crimes.committingCrime == CRIME_SLAVE_FREEING) {
            pc->crimes.committingCrime = CRIME_NONE;
            pc->crimes.crimeAgainstFaction = nullptr;
            pc->crimes.crimeExpiry = 0.0f;
            Log("BOUNTY_WATCH: cancelled slave-freeing charge on the player");
          }
          // The buyer picks up a bounty of his own for the freeing, and
          // that is what sent the guards after HIM while the freed man
          // walked away untouched. Only ever wipe it for the whole squad.
          if (bw->player) {
            for (size_t pi = 0; pi < bw->player->playerCharacters.size();
                 ++pi) {
              Character *sq = bw->player->playerCharacters[pi];
              if (!sq || (uintptr_t)sq < 0x1000)
                continue;
              int pb = sq->crimes.getTotalBounty();
              if (pb <= 0)
                continue;
              ...
              for (size_t bi2 = 0; bi2 < pf.size(); ++bi2)
                sq->crimes.clearBounty(pf[bi2]);
              sq->crimes.committingCrime = CRIME_NONE;
              sq->crimes.crimeAgainstFaction = nullptr;
              sq->crimes.crimeExpiry = 0.0f;
              Log("BOUNTY_WATCH: cleared " + ToString(pb) + " on squad member " +
                  sq->getName());
            }
          }
        }
      ...
      if (--w.ticksLeft <= 0)
        g_freedSlaves.erase(g_freedSlaves.begin() + fi);
      else
        ++fi;
    }
  }
    ...
    if (GetTickCount() - lastTalkTick > 500) {
      ...
      if (sel && (uintptr_t)sel > 0x1000) {
        ...
        if (!isMainPlayer) {
          ...
          //
          // endDialogue() used to be called here as well, and that is what
          // silently killed every speech bubble in chat: it tears the
          // dialogue system down, so the reply arriving a second later had
          // nothing left to draw into. Radiant NPCs never pass through this
          // window, which is why their bubbles kept working. Clearing the
          // in-dialog flag is enough to stop the vanilla window.
...
extern "C" __declspec(dllexport) void startPlugin() {
  ...
  ResetLogOnStartup();   // прежний лог — в копию, новый с чистого листа
