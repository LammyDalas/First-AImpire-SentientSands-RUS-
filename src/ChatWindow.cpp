...
MyGUI::ListBox *g_chatPreview = nullptr;
MyGUI::Button *g_chatModeBtns[6] = {nullptr, nullptr, nullptr,
...
void OnChatInputChange(MyGUI::EditBox *sender) {
  ...
  // Превью с переносом: поле EditBox длинный текст не показывает,
  // поэтому дублируем набранное в ListBox над полем, разбивая по
  // ширине. Кириллица считается по wide-символам, не по байтам.
  if (g_chatPreview) {
    g_chatPreview->removeAllItems();
    std::wstring w = Utf8ToWide(text);
    const size_t perLine = 60; // символов в строке превью
    if (w.empty()) {
      // пусто — ничего не показываем
    } else {
      for (size_t i = 0; i < w.size(); i += perLine)
        g_chatPreview->addItem(w.substr(i, perLine));
      g_chatPreview->setIndexSelected(MyGUI::ITEM_NONE);
    }
  }
}
      ...
      for (auto it = chars.begin(); it != chars.end(); ++it) {
        ...
        if (other && (uintptr_t)other > 0x1000 && other != player &&
          ...
          if (dist < searchRadius) {
            ...
            // Bondage state travels with every bystander. Without it the
            // server cannot tell a caged slave from a passer-by: captives
            // keep their original faction and personal name, so a slaver
            // standing next to his own cages reported having no stock.
            ...
            try {
              if (other->isDead()) {
                ...
                if (o_slave != 0)
                  o_state = other->isChainedMode() ? "enslaved"
                                                   : "escaped-slave";
              }
            } catch (...) {
            }
...
void CreateChatUI(const std::string &npcName, const std::string &playerName,
  ...
  g_chatWindow = gui->createWidgetReal<MyGUI::Window>(
      "Kenshi_WindowCX", 0.1875f, 0.32f, 0.625f, 0.34f, MyGUI::Align::Center,
  ...
  g_chatLabel->setCaption(Utf8ToWide(T("Message for ") + actualNpcName +
                                     T(" (from ") + playerName + T("):"))
                              .c_str());
  // Превью-строка над полем: показывает всю набранную фразу с переносом
  // (редактируемое поле скина длинный текст не отображает). Это ListBox,
  // как рабочее поле биографии в профиле — оно умеет многострочность.
  g_chatPreview = client->createWidgetReal<MyGUI::ListBox>(
      "Kenshi_ListBox", 0.05f, 0.24f, 0.9f, 0.30f,
      MyGUI::Align::Top | MyGUI::Align::HStretch, "SentientSands_ChatPreview");
  g_chatPreview->setNeedKeyFocus(false);

  g_chatInput = client->createWidgetReal<MyGUI::EditBox>(
      "Kenshi_EditBox", 0.05f, 0.58f, 0.9f, 0.16f,
      MyGUI::Align::Top | MyGUI::Align::HStretch, "SentientSands_ChatInput");

  // Поле ввода — простое, одну строку; читаемость обеспечивает превью выше.
  ...
  // --- Единая геометрия кнопок диалога ---
  const float BTN_Y = 0.82f;   // высота ряда
  const float BTN_W = 0.090f;  // ширина кнопки
  const float BTN_H = 0.11f;   // высота кнопки
  const float BTN_STEP = 0.101f; // шаг между кнопками

  // Сетка на 8 позиций от 0.050 до 0.950 — по краям поля ввода.
  // Позиции 1-6: способ обращения (шёпот, речь, крик, флирт, угроза, обмен).
  // Дальше отдельной группой: фоновый разговор и управление окном.
  const char *btnLabelKeys[] = {"Whisper", "Talk", "Yell",
                                "Flirt", "Threaten", "Trade"};
  float btnX = 0.050f;
  for (int i = 0; i < 6; i++) {
    g_chatModeBtns[i] = client->createWidgetReal<MyGUI::Button>(
        "Kenshi_Button1", btnX, BTN_Y, BTN_W, BTN_H,
  ...
  // Фоновый разговор — не режим обращения, поэтому в правой группе
  MyGUI::Button *radiantBtn = client->createWidgetReal<MyGUI::Button>(
      "Kenshi_Button1", 0.655f, BTN_Y, BTN_W, BTN_H,
      MyGUI::Align::Bottom | MyGUI::Align::Left,
  ...
  // --- Правая группа: отправка и отмена, отделены отступом ---

  MyGUI::Button *sendBtn = client->createWidgetReal<MyGUI::Button>(
      "Kenshi_Button1", 0.755f, BTN_Y, BTN_W, BTN_H,
      MyGUI::Align::Bottom | MyGUI::Align::Right, "SentientSands_ChatSendBtn");
  sendBtn->setCaption(Utf8ToWide(T("Send")).c_str());
  sendBtn->setColour(MyGUI::Colour(0.72f, 1.0f, 0.72f));
  sendBtn->setTextColour(MyGUI::Colour(0.72f, 1.0f, 0.72f));
  ...
  MyGUI::Button *cancelBtn = client->createWidgetReal<MyGUI::Button>(
      "Kenshi_Button1", 0.856f, BTN_Y, BTN_W, BTN_H,
      MyGUI::Align::Bottom | MyGUI::Align::Right,
      "SentientSands_ChatCancelBtn");
  cancelBtn->setCaption(Utf8ToWide(T("Cancel")).c_str());
  cancelBtn->setColour(MyGUI::Colour(1.0f, 0.72f, 0.72f));
  cancelBtn->setTextColour(MyGUI::Colour(1.0f, 0.72f, 0.72f));
...
void UpdateModeButtons() {
  ...
  for (int i = 0; i < 6; i++) {
    if (!g_chatModeBtns[i])
      continue;
    if ((size_t)i == g_lastChatModeIndex) {
      g_chatModeBtns[i]->setCaption(
          Utf8ToWide(std::string("> ") + T(btnLabelKeys[i]) + " <").c_str());
    } else {
      g_chatModeBtns[i]->setCaption(Utf8ToWide(T(btnLabelKeys[i])).c_str());
    }
  }
