...
// Characters the player has just bought out of slavery. The engine assigns
// the runaway bounty a moment AFTER the cage opens, so clearing it once at
// release time is too early: it has to be wiped again for a short while.
struct FreedSlaveWatch {
  hand who;
  int ticksLeft;
};
extern std::vector<FreedSlaveWatch> g_freedSlaves;
...
enum ActionType {
  ...
  ACT_RELEASE,
  ACT_TAKE_ITEM,
  ACT_SURRENDER,  // сложить оружие: погасить вражду и замереть
  ACT_SHOP_DEBUG  // разведка: где игра держит товар лавки (только запись в лог)
};
