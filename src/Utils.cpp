...
// ── Лог мода: перекладывание и предел ────────────────────────────────────────
// Раньше файл дописывался бесконечно и у игроков вырастал до сотен мегабайт.
// Теперь при запуске игры прежний лог уходит в запасную копию, а новый
// начинается с чистого листа. Если и он распухнет сверх предела — та же
// процедура повторяется на ходу. Одна копия сохраняется: по ней всё ещё можно
// разобрать, что было до переполнения.
static const char *kLogPath = "SentientSands_SDK.log";
static const char *kLogPathOld = "SentientSands_SDK.old.log";
static const long long kLogMaxBytes = 10LL * 1024 * 1024;  // 10 МБ

static void RotateLogFile() {
  // Прежняя копия уступает место более свежей.
  DeleteFileA(kLogPathOld);
  MoveFileA(kLogPath, kLogPathOld);
}

void ResetLogOnStartup() {
  EnterCriticalSection(&g_LogMutex);
  RotateLogFile();
  std::ofstream f(kLogPath, std::ios::trunc);
  if (f.is_open())
    f << "[SentientSands] LOG: новый файл (прежний сохранён как "
      << kLogPathOld << ")" << std::endl;
  LeaveCriticalSection(&g_LogMutex);
}

void Log(const std::string &msg) {
  EnterCriticalSection(&g_LogMutex);
  // Размер проверяем не на каждой строке, а изредка: обращение к файловой
  // системе стоит дорого, а лог пишется по многу раз в секунду.
  static unsigned int sinceCheck = 0;
  if (++sinceCheck >= 2000) {
    sinceCheck = 0;
    WIN32_FILE_ATTRIBUTE_DATA fad;
    if (GetFileAttributesExA(kLogPath, GetFileExInfoStandard, &fad)) {
      long long sz = ((long long)fad.nFileSizeHigh << 32) | fad.nFileSizeLow;
      if (sz > kLogMaxBytes)
        RotateLogFile();
    }
  }
  std::ofstream logFile(kLogPath, std::ios::app);
  if (logFile.is_open()) {
